package com.example.ourmemory.photoHelper;
...
public class LocalStorageProvider extends DocumentsProvider {
  ...
  private final static String[] DEFAULT_ROOT_PROJECTION = new String[] {
      ...
      Root.COLUMN_ICON, Root.COLUMN_AVAILABLE_BYTES };
  /**
  ...
  private final static String[] DEFAULT_DOCUMENT_PROJECTION = new String[] {
      Document.COLUMN_DOCUMENT_ID, Document.COLUMN_DISPLAY_NAME, Document.COLUMN_FLAGS,
  ...
  public String createDocument(final String parentDocumentId, final String mimeType,
    ...
    try {
      ...
      Log.e(LocalStorageProvider.class.getSimpleName(), "Error creating new file "
          + newFile);
    }
    return null;
  }
  ...
  public Cursor queryChildDocuments(final String parentDocumentId,
      final String[] projection, final String sortOrder) throws FileNotFoundException {
    // Create a cursor with either the requested fields, or the default
    // projection if "projection" is null.
    final MatrixCursor result = new MatrixCursor(projection != null ? projection
  ...
  private void includeFile(final MatrixCursor result, final File file)
    ...
    int flags = file.canWrite() ? Document.FLAG_SUPPORTS_DELETE
